...
That shape of failure is worth naming, because it recurs: a location that cannot be read, a descriptor nothing watches, an enrolled component with no verdict yet. Each is *correct and invisible*, and Backstage logs every one of them where nobody looks. If your instance has somewhere to send metrics, pointing a dashboard at the catalog's own signals — unresolvable locations, processing errors, entity counts by kind — turns that class of problem from a mystery into a number. Nothing here ships one yet.
